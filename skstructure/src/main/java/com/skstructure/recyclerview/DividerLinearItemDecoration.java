...
public class DividerLinearItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public DividerLinearItemDecoration(Drawable divider, int dividerSize) {
        set(divider, dividerSize, new ColorDrawable(Color.TRANSPARENT), 0);
    }
    ...
    public DividerLinearItemDecoration(@ColorInt int dividerColor, int dividerSize) {
        set(new ColorDrawable(dividerColor), dividerSize, new ColorDrawable(Color.TRANSPARENT), 0);
    }

    /**
     * @param dividerColor     分隔线的颜色
     * @param dividerSize      分隔线的大小
     * @param lastDividerColor 最后一行的分隔线颜色
     * @param lastDividerSize  最后一行分隔线的大小，如果小于等于0，则不画
     */
